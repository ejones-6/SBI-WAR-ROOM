...
export default function BoePanel({ deal, boe, onSave }: Props) {
  ...
  function handleExport() {
    ...
    const rows: any[][] = [
      ...
      [],
      [null, 'EXPENSES'],
      [null, 'General & Administrative',   'T12', ga_t,  fmtpu(ga_t,units),  ga_p - ga_t,  ga_p,  fmtpu(ga_p,units),  notes['ga']  || ''],
      [null, 'Marketing / Advertising',    'T12', mkt_t, fmtpu(mkt_t,units), mkt_p - mkt_t,mkt_p, fmtpu(mkt_p,units), notes['mkt'] || ''],
      [null, 'Repairs & Maintenance',      'T12', rm_t,  fmtpu(rm_t,units),  rm_p - rm_t,  rm_p,  fmtpu(rm_p,units),  notes['rmi-rm'] ? `$${rmi['rmi-rm']}/unit R&M · $${rmi['rmi-ct']}/unit Contracts · $${rmi['rmi-tu']}/unit Turnover` : ''],
      [null, 'Payroll',                    'T12', pay_t, fmtpu(pay_t,units), pay_p - pay_t, pay_p, fmtpu(pay_p,units), notes['pay'] || ''],
      [null, 'Controllable Expenses',      null,  ctrl_t, fmtpu(ctrl_t,units), null, ctrl_p, fmtpu(ctrl_p,units)],
      [],
      [null, 'Property Management Fee',    'T12', mgt_t, fmtpu(mgt_t,units), mgt_p - mgt_t, mgt_p, fmtpu(mgt_p,units), adjs['mgt'] ? `${adjs['mgt']}% of EGR` : ''],
      [null, 'Utilities',                  'T12', utl_t, fmtpu(utl_t,units), utl_p - utl_t, utl_p, fmtpu(utl_p,units), notes['utl'] || ''],
      [null, 'Real Estate Taxes',          'T12', tax_t, fmtpu(tax_t,units), tax_p - tax_t, tax_p, fmtpu(tax_p,units), notes['tax'] || ''],
      [null, 'Insurance',                  'T12', ins_t, fmtpu(ins_t,units), ins_p - ins_t, ins_p, fmtpu(ins_p,units), adjs['ins'] ? `$${adjs['ins']}/unit` : ''],
